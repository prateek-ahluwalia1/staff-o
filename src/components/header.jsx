...
const Header = memo(function Header({ withSidebar = false }) {
  ...
  const renderUserAvatar = () => {
    ...
    if (imageUrl) {
      return <img src={imageUrl} alt="Profile" style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} onError={(e) => { e.target.style.display = "none"; }} />;
    }
    return (
      <div style={{ width: "100%", height: "100%", borderRadius: "50%", backgroundColor: getAvatarColor(displayName), display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: "bold", fontSize: "0.9rem" }}>
  ...
  return (
    <div className={`header ${sidebarHeaderClass}`.trim()}>
              ...
              <div className="desktop-actions-wrapper d-flex align-items-center gap-4">
                ...
                <div className="notification-wrapper position-relative">
                  ...
                  >
                    ...
                    {unreadCount > 0 && (
                      <span style={{ position: 'absolute', top: '-5px', right: '-5px', backgroundColor: '#dc3545', color: 'white', borderRadius: '50%', width: '18px', height: '18px', fontSize: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
                        {unreadCount}
                      </span>
                    )}
                  </button>
                  ...
                  {showNotifications && (
                    <div className="dropdown-menu dropdown-menu-end show shadow notification-dropdown-menu" style={{ position: "absolute", right: 0, top: "40px", display: "block", padding: "0", width: "300px" }}>
                      ...
                      <ul className="list-unstyled mb-0" style={{ maxHeight: "300px", overflowY: "auto" }}>
                        {items.length > 0 ? (
                          items.map((notif, index) => (
                            <li key={notif.id || index} className="p-3 border-bottom dropdown-item" style={{ whiteSpace: "normal" }} role="button" onClick={() => markSingleNotificationRead(notif)}>
                              <div className="small text-dark fw-bold">{getNotificationTitle(notif)}</div>
                              <div className="small text-muted mt-1" style={{ textTransform: "none" }}>{getNotificationMessage(notif)}</div>
                            </li>
                          ))
                        ) : (
                      ...
                      <div className="p-2 text-center border-top bg-light">
                        <NavLink to="/notifications" className="small text-primary text-decoration-none fw-bold" onClick={() => setShowNotifications(false)}>View All</NavLink>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="dropdown user-dropdown desktop-user-dropdown" style={{ display: "flex", alignItems: "center" }}>
                  <button className="btn dropdown-toggle p-0" type="button" data-bs-toggle="dropdown" style={{ display: "flex", alignItems: "center", gap: "10px", background: "transparent", border: "none", boxShadow: "none", color: "#333" }}>
                    <div style={{ width: "40px", height: "40px", borderRadius: "50%", overflow: "hidden", flexShrink: 0 }}>
                      {renderUserAvatar()}
                    </div>
                    <span style={{ fontWeight: "600", fontSize: "15px", whiteSpace: "nowrap", display: "block" }}>
                      {displayName}
                    </span>
                  </button>
                  ...
                  <ul className="dropdown-menu dropdown-menu-end shadow-sm mt-2">
                    ...
                    <li>
                      <button type="button" className="dropdown-item py-2 text-danger" onClick={async () => {
                        try {
      ...
      {/* CSS Logic to hide desktop elements on mobile devices */}
      <style>{`
