...
export default function ReviewStep({ form, rate, setField, handleConfirm, setStep, isSubmitting, baseAmount }) {
  ...
  return (
    <div className="bg-white rounded-4 p-4 border" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.03)" }}>
      ...
      <div className="mb-4 pt-3 border-top">
        <div className="d-flex align-items-center gap-2 mb-3"><i className="fa-solid fa-credit-card text-primary fs-5"></i><h6 className="fw-bold mb-0">Payment Options</h6></div>
      ...
      <div className="d-flex flex-column flex-md-row justify-content-end gap-3 mt-4 pt-3 border-top">
        ...
        <button className="btn btn-success fw-bold px-5 py-2 shadow-sm order-1 order-md-2 d-flex align-items-center justify-content-center gap-2" onClick={handleConfirm} disabled={!form.termsAccepted || isSubmitting}>
          {isSubmitting ? (<><span className="spinner-border spinner-border-sm" aria-hidden="true"></span><span role="status">Processing...</span></>) : (<><i className="fa-brands fa-stripe fs-5"></i> Pay {fmt(activeAmount)} & Post</>)}
        </button>
      </div>
    </div>
  );
